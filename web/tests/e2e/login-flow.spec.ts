...
test.describe("login and logout shell flow", () => {
  test("login page shows disabled SSO options when providers are not configured", async ({ page }) => {
    ...
    await expect(page.getByRole("main", { name: "Sign in to CloudStack" })).toBeVisible();
    ...
    await expect(page.getByRole("group", { name: "Identity provider sign-in options" })).toBeVisible();
    ...
    await expect(page.getByRole("status")).toHaveText("Configure an identity provider to enable single sign-on.");
  });

  test("login page announces identity provider failures", async ({ page }) => {
    await page.goto("/login?error=AccessDenied");

    const loginMain = page.getByRole("main", { name: "Sign in to CloudStack" });
    await expect(loginMain.getByRole("alert")).toHaveText("Access was denied by the identity provider.");

...
class TestCreateVolume(cloudstackTestCase):
    ...
    def test_01_create_volume(self):
        ...
        for volume in self.volumes:
            ...
            elif list_volume_response[0].hypervisor.lower() == "proxmox":
                # disks are attached on a virtio-scsi bus: sda is the root disk,
                # data disks follow by device id
                volume_name = "/dev/sd" + chr(ord('a') + int(list_volume_response[0].deviceid))
                self.debug(" Using Proxmox volume_name: %s" % (volume_name))
                ret = checkVolumeSize(ssh_handle=ssh, volume_name=volume_name, size_to_verify=vol_sz)
            else:
